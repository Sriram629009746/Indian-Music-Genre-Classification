...
if __name__ == "__main__":
    ...
    for k in range(2,20):
        ...
        print ( "k = {} , train: {}, cv : {}, test : {}".format(k,train_accuracy,np.mean(scores),test_accuracy))
        if(np.mean(scores) > maxcvscore):
            maxcvscore = np.mean(scores)
            kmin = k

    model = KNeighborsClassifier(n_neighbors=kmin)
    model.fit(X_train, y_train)
    train_accuracy = model.score(X_train, y_train)
    test_accuracy = model.score(X_test, y_test)
    print ("final k : {}".format(kmin))
    print ("Final Training Accuracy:{}".format(train_accuracy))
    print("Final Testing Accuracy:{}".format(test_accuracy))
    ...
    joblib.dump(model, 'knn_indian_final.rar')
